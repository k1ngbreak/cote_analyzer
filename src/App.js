...
function Spinner() {
  return (
    <div style={{ display: "inline-block", width: 16, height: 16, border: "2px solid #3a3a55", borderTopColor: "#a78bfa", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
  );
}
...
export default function App() {
  ...
  const a1 = analyzeP(p1);
  const a2 = analyzeP(p2);
  ...
  const extractPatterns = () => {
    ...
    history.forEach((m) => {
      ...
      if (!map[k]) {
        map[k] = { fav: 0, outsider: 0, meta: { a1: favA, a2: outA }, lastWinner: lastW, favOdds: [], outsiderOdds: [], roundStats: {}, _winnerIsFavArr: [] };
      }
    ...
    Object.entries(map).forEach(([, v]) => {
      ...
      if (conf >= extractConf) {
        out.push({ ...v, total, winner: bestW, confidence: conf, avgFavOdd: avg(v.favOdds), avgOutsiderOdd: avg(v.outsiderOdds), avgWinOddWhenCorrect, topRounds, totalWithRound });
      }
    });
  ...
  const alreadyAddedGolden = (p) => rules.some((r) =>
    ...
    r.last_winner === (p.lastWinner !== "inconnu" ? p.lastWinner : undefined)
  );
  ...
  const addGoldenRule = (p) => {
    ...
    setRules(prevRules => {
      const updated = [...prevRules, {
        ...
        label,
        description: `Deep Scan — ${p.total} matchs, ${p.confidence}% réussite. Seuils : Hausse >${p.thUp}, Baisse <${p.thDown}.`,
        p1_movement: p.a1.movement,
        p1_breach: p.a1.breach,
        p2_movement: p.a2.movement,
        p2_breach: p.a2.breach,
      ...
      if (rulesRowId) sbSet("rules", rulesRowId, updated).catch(() => {});
      return updated;
    });
  };

  const normalizeHistory = () => {
    if (!window.confirm("Ceci va réorganiser tes matchs pour que J1 soit toujours le favori. Continuer ?")) return;
    const fixedHistory = history.map(m => {
      const p1Fav = getP1IsFav(m);
      if (p1Fav) return { ...m, p1IsFav: true };
      let newWinner = m.winner;
      if (m.winner === "p1") newWinner = "p2";
      else if (m.winner === "p2") newWinner = "p1";
      return { ...m, a1: m.a2, a2: m.a1, winner: newWinner, p1IsFav: true };
    });
    saveHistory(fixedHistory);
    alert("Historique corrigé !");
  };
  ...
  const alreadyAdded = (s) => rules.some((r) =>
    ...
    r.last_winner === (s.lastWinner !== "inconnu" ? s.lastWinner : undefined)
  );
  ...
  const addSuggested = (s) => {
    const lwPart = s.lastWinner && s.lastWinner !== "inconnu" ? ` + Dernier:${s.lastWinner === "favori" ? "Fav" : "Out"}` : "";
    const label = `Favori ${s.meta.a1.movement === "up" ? "monte" : "baisse"} (${s.meta.a1.breach ? "seuil KO" : "seuil OK"}) + Outsider ${s.meta.a2.movement === "up" ? "monte" : "baisse"} (${s.meta.a2.breach ? "seuil KO" : "seuil OK"})${lwPart} → ${s.winner === "favori" ? "Favori" : "Outsider"} gagne`;
    ...
    if (existing) {
      ...
      saveRules([...rules, {
        ...
        label,
        description: `Détectée auto — ${s.total} matchs, confiance ${s.confidence}%`,
        p1_movement: s.meta.a1.movement,
        p1_breach: s.meta.a1.breach,
        p2_movement: s.meta.a2.movement,
        p2_breach: s.meta.a2.breach,
        last_winner: s.lastWinner !== "inconnu" ? s.lastWinner : undefined,
        winner: s.winner,
        active: true,
        confidence: s.confidence,
          ...
          <>
            ...
            {tab === "analyze" && (
              <>
                  ...
                  <div style={{ marginTop: "1rem" }}>
                    ...
                    <div style={{ display: "flex", gap: "0.5rem" }}>
                      {["favori", "outsider"].map(v => (
                        <button
                          key={v}
                          onClick={() => setLastWinnerInput(v)}
                          style={{ flex: 1, padding: "0.55rem", borderRadius: 6, border: `1px solid ${lastWinnerInput === v ? (v === "favori" ? "#f59e0b" : "#60a5fa") : "#2a2a3a"}`, background: lastWinnerInput === v ? (v === "favori" ? "#1c1008" : "#0a1018") : "transparent", color: lastWinnerInput === v ? (v === "favori" ? "#fbbf24" : "#93c5fd") : "#6b6b88", fontFamily: "IBM Plex Mono, monospace", fontSize: "0.78rem", cursor: "pointer", fontWeight: lastWinnerInput === v ? 600 : 400 }}
                        >
                    ...
                    {matchedRules.length > 0 ? (
                      <div>
                        {matchedRules.map((r) => {
                          ...
                          return (
                            <div key={r.id} style={{ background: isGolden ? "linear-gradient(135deg,#2a1600,#140a00)" : "#0d1f0d", border: `1px solid ${isGolden ? "#f59e0b" : "#1e3a1e"}`, borderRadius: 8, padding: "0.85rem 1rem", marginBottom: "0.75rem", animation: isGolden ? "pulse-gold 2s infinite" : "none" }}>
                ...
                {!hasAnalysis && (
                  <div style={{ textAlign: "center", padding: "2rem", color: "#6b6b88", fontSize: "0.8rem" }}>
                    Saisis les cotes avant/après pour les deux joueurs.
                  </div>
                )}
              </>
            )}
                          ...
                          <div key={i} style={{ background: "#0a0f0a", border: "1px solid #1e3a1e", borderRadius: 10, padding: "1rem 1.25rem", marginBottom: "0.65rem", display: "flex", alignItems: "flex-start", gap: "1rem" }}>
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap", marginBottom: "0.4rem" }}>
                                <span style={{ background: "#1a1a2e", border: "1px solid #2a2a3a", borderRadius: 4, padding: "0.2rem 0.5rem", fontSize: "0.65rem", color: "#a0a0c0" }}>
                                  Favori: {s.meta.a1.movement === "up" ? "monte" : "baisse"} {s.meta.a1.breach ? "⚠" : "✓"}
                                </span>
                                <span style={{ background: "#1a1a2e", border: "1px solid #2a2a3a", borderRadius: 4, padding: "0.2rem 0.5rem", fontSize: "0.65rem", color: "#a0a0c0" }}>
                                  Outsider: {s.meta.a2.movement === "up" ? "monte" : "baisse"} {s.meta.a2.breach ? "⚠" : "✓"}
                                </span>
                              ...
                              <div style={{ marginBottom: "0.4rem" }}>
                                <span style={{ background: "#0f1a0f", border: "1px solid #166534", borderRadius: 4, padding: "0.2rem 0.5rem", fontSize: "0.65rem", color: "#4ade80", fontWeight: 600 }}>
                                  ✓ Cote moy. {s.winner === "favori" ? "Favori" : "Outsider"} quand correct : {s.avgWinOddWhenCorrect}
                                </span>
                              </div>
                              {s.topRounds && s.topRounds.length > 0 && (
                                <div style={{ marginTop: "0.3rem" }}>
                                  <span style={{ fontSize: "0.62rem", color: "#6b6b88", textTransform: "uppercase", letterSpacing: "0.06em" }}>Rounds ({s.totalWithRound}/{s.total} renseignés) : </span>
                                  <div style={{ display: "flex", gap: "0.3rem", flexWrap: "wrap", marginTop: "0.3rem" }}>
                  ...
                  {history.length === 0 ? (
                    ...
                    return (
                      <div key={m.id} style={{ background: "#0a0a0f", border: "1px solid #1e1e30", borderRadius: 10, padding: "1rem 1.25rem", marginBottom: "0.75rem" }}>
                        ...
                        <div className="g2">
                          {[
                            { label: "Favori", a: m.a1, color: "#a78bfa" },
                            { label: "Outsider", a: m.a2, color: "#60a5fa" },
                          ].map((p, i) => (
            ...
            {tab === "kb" && (
              <div style={S.section}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                  ...
                  <button style={{ ...S.btn, background: "linear-gradient(135deg,#7c3aed,#2563eb)", color: "white" }} onClick={() => setShowForm(!showForm)}>
                  ...
                  <div style={{ background: "#0a0a0f", border: "1px solid #2a2a3a", borderRadius: 10, padding: "1.25rem", marginBottom: "1.25rem" }}>
                    ...
                    <div className="fg" style={{ marginBottom: "0.75rem" }}>
                      <div style={{ gridColumn: "1/-1" }}>
                        ...
                        <input className="inp" type="text" placeholder="ex: Favori monte (seuil KO) → Outsider gagne" value={newRule.label} onChange={(e) => setNewRule({ ...newRule, label: e.target.value })} />
                      </div>
                      <div style={{ gridColumn: "1/-1" }}>
                        <label style={S.label}>Description</label>
                        <textarea className="inp" style={{ minHeight: 60, resize: "vertical", lineHeight: 1.5 }} placeholder="Explication..." value={newRule.description} onChange={(e) => setNewRule({ ...newRule, description: e.target.value })} />
                      </div>
                      <div>
                        <label style={S.label}>Favori — Mouvement</label>
                        <select value={newRule.p1_movement} onChange={(e) => setNewRule({ ...newRule, p1_movement: e.target.value })}>
                          <option value="up">▲ Monte</option>
                          <option value="down">▼ Baisse</option>
                          <option value="any">Peu importe</option>
                        </select>
                      </div>
                      <div>
                        <label style={S.label}>Favori — Seuil</label>
                        <select value={newRule.p1_breach ? "b" : "o"} onChange={(e) => setNewRule({ ...newRule, p1_breach: e.target.value === "b" })}>
                          <option value="b">⚠ Non-respecté</option>
                          <option value="o">✓ Respecté</option>
                        </select>
                      </div>
                      <div>
                        <label style={S.label}>Outsider — Mouvement</label>
                        <select value={newRule.p2_movement} onChange={(e) => setNewRule({ ...newRule, p2_movement: e.target.value })}>
                          <option value="up">▲ Monte</option>
                          <option value="down">▼ Baisse</option>
                          <option value="any">Peu importe</option>
                        </select>
                      ...
                      <div>
                        <label style={S.label}>Outsider — Seuil</label>
                        <select value={newRule.p2_breach ? "b" : "o"} onChange={(e) => setNewRule({ ...newRule, p2_breach: e.target.value === "b" })}>
                          <option value="b">⚠ Non-respecté</option>
                          <option value="o">✓ Respecté</option>
                        </select>
                      </div>
                      <div style={{ gridColumn: "1/-1" }}>
                        <label style={S.label}>Gagnant prédit</label>
                        <select value={newRule.winner} onChange={(e) => setNewRule({ ...newRule, winner: e.target.value })}>
                          <option value="favori">Favori</option>
                          <option value="outsider">Outsider</option>
                        </select>
                      </div>
                  ...
                  <div key={r.id} style={{ background: "#0a0a0f", border: "1px solid #1e1e30", borderRadius: 10, padding: "1rem 1.25rem", marginBottom: "0.75rem", display: "flex", alignItems: "flex-start", gap: "1rem", opacity: r.active === false ? 0.45 : 1 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                        <span style={{ background: "#1a1a2e", border: "1px solid #2a2a3a", borderRadius: 4, padding: "0.2rem 0.5rem", fontSize: "0.65rem", color: "#a0a0c0" }}>
                          Favori: {r.p1_movement === "up" ? "monte" : r.p1_movement === "down" ? "baisse" : "~"} {r.p1_breach ? "⚠" : "✓"}
                        </span>
                        <span style={{ background: "#1a1a2e", border: "1px solid #2a2a3a", borderRadius: 4, padding: "0.2rem 0.5rem", fontSize: "0.65rem", color: "#a0a0c0" }}>
                          Outsider: {r.p2_movement === "up" ? "monte" : r.p2_movement === "down" ? "baisse" : "~"} {r.p2_breach ? "⚠" : "✓"}
                        </span>
                        {r.last_winner && (
                          <span style={{ background: "#0a1018", border: "1px solid #1e3a5e", borderRadius: 4, padding: "0.2rem 0.5rem", fontSize: "0.65rem", color: "#93c5fd" }}>
                            ⏮ Dernier: {r.last_winner === "favori" ? "Fav" : "Out"}
                          </span>
                        )}
                        <span style={{ background: "#1a1a2e", border: "1px solid #2a2a3a", borderRadius: 4, padding: "0.2rem 0.5rem", fontSize: "0.65rem", color: "#fbbf24" }}>
                        ...
                        {r.confidence && (
                          <span style={{ background: "#1a1a2e", border: "1px solid #2a2a3a", borderRadius: 4, padding: "0.2rem 0.5rem", fontSize: "0.65rem", color: "#a78bfa" }}>
                            🤖 {r.confidence}%
                          </span>
                        )}
                      </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem", flexShrink: 0 }}>
                      <button onClick={() => saveRules(rules.map(x => x.id === r.id ? { ...x, active: !x.active } : x))} style={{ background: "transparent", border: `1px solid ${r.active !== false ? "#4ade80" : "#f87171"}`, borderRadius: 4, color: r.active !== false ? "#4ade80" : "#f87171", fontFamily: "IBM Plex Mono, monospace", fontSize: "0.65rem", padding: "0.3rem 0.6rem", cursor: "pointer" }}>
            ...
            {tab === "settings" && (
              <div style={S.section}>
                ...
                <div style={{ ...S.section, marginTop: "1.25rem", border: "1px solid #7f1d1d", background: "#2a0a0a" }}>
                  ...
                  <button style={{ ...S.btn, background: "#7f1d1d", color: "white" }} onClick={normalizeHistory}>
